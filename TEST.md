...
| 6 | joker.spec.ts | Joker button › stays hidden until enabled in the side menu | Active |
| 7 | joker.spec.ts | Joker button › stamps a Joker and resolves it to a bib using the original captured time, undo works | Active |
| 8 | joker.spec.ts | Joker button › blocks resolving a Joker to a bib with a more recent lap already recorded | Active |
| 9 | joker.spec.ts | Joker button › Delete removes an unresolved Joker without touching any rider | Active |
| 10 | lap-recording.spec.ts | Lap recording core › records a lap, blocks a second within 60s, and undoes cleanly | Active |
| 11 | lap-recording.spec.ts | Lap recording core › board hold freezes the grid during a burst, then drops every tapped card at once | Active |
| 12 | lap-recording.spec.ts | Lap recording core › undo inside the hold cancels the pending move and clears the ✓ | Active |
| 13 | lap-recording.spec.ts | Lap recording core › cancelling from the rider log inside the hold also cancels the pending move | Active |
| 14 | lap-recording.spec.ts | Lap recording core › the live-screen gear changes the hold, and the new value applies straight away | Active |
| 15 | lap-recording.spec.ts | Lap recording core › max board hold flushes anyway, so a nonstop trickle can't freeze the board forever | Active |
| 16 | lap-recording.spec.ts | Lap recording core › the rider-log button sits in the timer row, aligned with the clock | Active |
| 17 | lap-recording.spec.ts | Lap recording core › DNF and DSQ move a rider out of the racing grid | Active |
| 18 | race-flow.spec.ts | Commissaire race flow › add riders, schedule, start & finish a wave, see results | Active |
| 19 | rider-edits.spec.ts | Rider edits › change a rider's name and start time without breaking | Active |
| 20 | side-menu.spec.ts | Side menu › offers a direct feedback email | Active |
| 21 | side-menu.spec.ts | Side menu › board hold defaults to 2s and persists the chosen value | Active |
| 22 | terms-gate.spec.ts | Terms gate (landing page) › blocks the CTA until the box is ticked, then remembers the acceptance | Active |
| 23 | terms-gate.spec.ts | Terms gate (landing page) › the terms text is reachable from the landing page | Active |
| 24 | time-parsing.spec.ts | Shared clock-time parsing › parses the formats the app actually stores | Active |
| 25 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › schedule groups the field into two waves | Active |
| 26 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › schedule lists all eight categories | Active |
| 27 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › race mode offers a pill for each wave | Active |
| 28 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › live board shows wave 1's running categories | Active |
| 29 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › live grid gives the category top 3 gold / silver / bronze badges | Active |
| 30 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › upcoming and other-wave riders are not on the live grid | Active |
| 31 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › standings page opens a category leaderboard | Active |
| 32 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › results show HH:MM:SS finisher times after the wave finishes | Active |
| 33 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › the category winner reads as P1 on the board and first in results | Active |
| 34 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › results wave filter separates wave 1 from wave 2 | Active |
