# Commissaire Test Catalogue

## Run Commands

- `npm test` refreshes the active Playwright inventory below, then runs every E2E test.
- `npm run test:list` prints the exact tests Playwright will run without executing them.
- `npm run test:full` runs the complete race-day scenario only.
- `npm run test:e2e:headed` opens Chromium while the full suite runs.

The generated inventory is the source of truth for **active** E2E tests. The
100-case plan is a delivery checklist: `Planned` entries are intentional work,
not passing tests.

<!-- PLAYWRIGHT-INVENTORY:START -->
...
**Active automated cases: 38**

| # | Spec | Playwright test | Status |
| --- | --- | --- | --- |
| 1 | create-race.spec.ts | Create race › creates a race with no riders file | Active |
| 2 | csv-column-order.spec.ts | CSV column order › header-mapped import is independent of column order | Active |
| 3 | csv-import.spec.ts | CSV upload › import riders from a CSV file | Active |
| 4 | dns-dnf.spec.ts | DNS / DNF status › toggle DNS and DNF from the schedule without breaking | Active |
| 5 | full-race-e2e.spec.ts | Full race day › build a race, run both waves, and export a signed result file | Active |
| 6 | i18n.spec.ts | i18n › translation keys are flat strings, not nested-object paths | Active |
| 7 | i18n.spec.ts | i18n › Scan Start List is disabled with a Soon badge (Riders actions menu) | Active |
| 8 | i18n.spec.ts | i18n › switching language in the side menu updates visible UI text live | Active |
| 9 | joker.spec.ts | Joker button › stays hidden until enabled in the side menu | Active |
| 10 | joker.spec.ts | Joker button › stamps a Joker and resolves it to a bib using the original captured time, undo works | Active |
| 11 | joker.spec.ts | Joker button › blocks resolving a Joker to a bib with a more recent lap already recorded | Active |
| 12 | joker.spec.ts | Joker button › Delete removes an unresolved Joker without touching any rider | Active |
| 13 | lap-recording.spec.ts | Lap recording core › records a lap, blocks a second within 60s, and undoes cleanly | Active |
| 14 | lap-recording.spec.ts | Lap recording core › board hold freezes the grid during a burst, then drops every tapped card at once | Active |
| 15 | lap-recording.spec.ts | Lap recording core › undo inside the hold cancels the pending move and clears the ✓ | Active |
| 16 | lap-recording.spec.ts | Lap recording core › cancelling from the rider log inside the hold also cancels the pending move | Active |
| 17 | lap-recording.spec.ts | Lap recording core › the live-screen gear changes the hold, and the new value applies straight away | Active |
| 18 | lap-recording.spec.ts | Lap recording core › max board hold flushes anyway, so a nonstop trickle can't freeze the board forever | Active |
| 19 | lap-recording.spec.ts | Lap recording core › the rider-log button sits in the timer row, aligned with the clock | Active |
| 20 | lap-recording.spec.ts | Lap recording core › DNF and DSQ move a rider out of the racing grid | Active |
| 21 | race-flow.spec.ts | Commissaire race flow › add riders, schedule, start & finish a wave, see results | Active |
| 22 | race-status.spec.ts | effectiveRaceStatus › treats a past-dated upcoming race as finished, leaves others alone | Active |
| 23 | rider-edits.spec.ts | Rider edits › change a rider's name and start time without breaking | Active |
| 24 | side-menu.spec.ts | Side menu › offers a direct feedback email | Active |
| 25 | side-menu.spec.ts | Side menu › board hold defaults to 2s and persists the chosen value | Active |
| 26 | terms-gate.spec.ts | Terms gate (landing page) › blocks the CTA until the box is ticked, then remembers the acceptance | Active |
| 27 | terms-gate.spec.ts | Terms gate (landing page) › the terms text is reachable from the landing page | Active |
| 28 | time-parsing.spec.ts | Shared clock-time parsing › parses the formats the app actually stores | Active |
| 29 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › schedule groups the field into two waves | Active |
| 30 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › schedule lists all eight categories | Active |
| 31 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › race mode offers a pill for each wave | Active |
| 32 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › live board shows wave 1's running categories | Active |
| 33 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › live grid gives the category top 3 gold / silver / bronze badges | Active |
| 34 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › upcoming and other-wave riders are not on the live grid | Active |
| 35 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › standings page opens a category leaderboard | Active |
| 36 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › results show HH:MM:SS finisher times after the wave finishes | Active |
| 37 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › the category winner reads as P1 on the board and first in results | Active |
| 38 | waves-leaderboard.spec.ts | Waves, leaderboard, winners & finisher times › results wave filter separates wave 1 from wave 2 | Active |
<!-- PLAYWRIGHT-INVENTORY:END -->

## 100-Case Coverage Plan

| IDs       | Area                                                                                                                                                                                                            | Cases | State   |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------- |
| T001-T015 | Existing active Playwright cases listed above                                                                                                                                                                   | 15    | Active  |
| T016-T025 | Race creation and persistence: required fields, date/location, no riders, refresh, duplicate names, delete, downloaded/view-only removal, race-mode resume, bad file feedback, error recovery                   | 10    | Planned |
| T026-T040 | CSV/XLSX import: UTF-8 Hebrew, aliases, shuffled columns, info fields, duplicate bibs, missing headers, malformed rows, template download, mapping save/load, preview, cancel, re-import, club dictionary, XLSX | 15    | Planned |
| T041-T050 | Categories and schedule: derivation, flat categories, category laps sync, manual color, auto color, waves, staggered starts, edit, delete protection, standing navigation                                       | 10    | Planned |
| T051-T060 | Check-in and start: per-rider check-in, check-all, DNS, undo DNS, late entry, start validation, staggered groups, start all, finish group, finish wave                                                          | 10    | Planned |
| T061-T075 | Live lap recording: first lap, debounce, 60-second guard, completion, queue order, undo, DNF, DSQ, DNS exclusion, notes, reload persistence, status precedence, clear board, search, Joker                      | 15    | Planned |
| T076-T085 | Results and exports: category positions, overall positions, elapsed time, status rows, column picker persistence, CSV/XLSX export, signature valid, signature modified, unsigned import, downloaded results     | 10    | Planned |
| T086-T092 | Offline and data safety: IndexedDB reload, migration error preserves data, localStorage recovery, action-log wave isolation, Joker wave isolation, corrupt cache handling, clear-data confirmation              | 7     | Planned |
| T093-T100 | Responsive and visual regression: mobile nav, 320px live grid, timer/icon overlap, touch target sizes, Hebrew RTL, desktop results, modal overflow, screenshot baselines                                        | 8     | Planned |

### Acceptance Rules

1. Each new test is given a `T###` ID in its test title and is automatically
   included in the generated inventory once committed.
2. A case moves from `Planned` to `Active` only after it has a Playwright test
   and passes in `npm test`.
3. CSS cases use Playwright screenshots at desktop and mobile viewports; logic
   cases assert visible user outcomes and persistence, not implementation detail.
4. Any bug fix adds or updates a regression case before the item is marked done.

---

## Full Race-Day Scenario

**Spec:** `tests/full-race-e2e.spec.ts`
**Start list:** `tests/fixtures/full-race.csv`

The full-race scenario creates a two-wave event, imports riders, checks riders
in, records laps and statuses, validates boards/results, then exports and
verifies a signed workbook. It uses Playwright's fake clock so timing behavior is
deterministic.

### Current Known Coverage Gaps

The active suite does not yet automate OCR/photo import, map behavior, cloud
sync, PWA installation, multi-day races, or visual regression. Those areas are
represented in the 100-case plan above and should stay `Planned` until backed by
passing tests.
...
The following older notes are retained for scenario background. The generated
inventory above replaces their previous test-count claims.
...
bug. The riders still out there are _supposed_ to stay on the grid with the
...
**Finish Wave** → **Yes** → the wave reports _Wave 1 finished_. Then the entire
...
- **Different field size or lap counts** → edit `tests/fixtures/full-race.csv`
  _and_ the matching `WAVE_1` / `WAVE_2` blocks in the spec (the spec asserts
...
| ----- | -------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| L6    | ⚑ ON TRACK is unreachable dead code                                        | **Open**                      | `StartManager.tsx:594,653,685` still set `raceStatus: "finished"` on every rider. Also proven by the test _passing_ — see note below |
...
The underlying _product_ gaps behind U5 and U6 are still open — there's still no
...
| 20   | Two divergent import paths (was L3)             | ✅ Fixed — one detector _and_ one row builder |
...
- **22** was two bugs: the selection was discarded _and_ the modal only ever
...
in the set, so the safety net went in _first_: `lap-recording.spec.ts` was
...
`FIN 3/4` — recorded as _finishers_ despite never completing the distance.
...
`raceStatus: "running"` (only closing the _category_), and let the existing
