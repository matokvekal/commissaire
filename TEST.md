...
| 11 | lap-recording.spec.ts | Lap recording core › DNF and DSQ move a rider out of the racing grid | Active |
| 12 | race-flow.spec.ts | Commissaire race flow › add riders, schedule, start & finish a wave, see results | Active |
| 13 | rider-edits.spec.ts | Rider edits › change a rider's name and start time without breaking | Active |
| 14 | side-menu.spec.ts | Side menu › offers a direct feedback email | Active |
| 15 | time-parsing.spec.ts | Shared clock-time parsing › parses the formats the app actually stores | Active |
